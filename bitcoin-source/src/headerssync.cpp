...
// GPU-mined-crypto note: CompressedHeader grew from 48 to 88 bytes (added
// nHeight/mix_hash, widened nonce to 64 bits - see primitives/block.h) -
// the upstream headerssync-params.py memory-budget analysis this assert
// originally cross-checked against was never re-run for the new size
// (would need real profiling against this project's own memory targets,
// not attempted here - flagged in NOTES.md as a known follow-up, not a
// blocker to a working header-sync implementation).
